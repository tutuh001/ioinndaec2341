                    ...
                    if (err) {
                        ...
                        if (safeGet(data)) {
                            ...
                            if (data.code === 0) {
                                ...
                                if (data.data) {
                                    let {
                                        alreadySignDays, beanTotalNum, todayPrize, eachDayPrize
                                    } = data.data;
                            ...
                            if (data['success']) {
                                if (data['data'] && data['data']['floorInfoList']) {
                                    ...
                                    if (floorInfo.hasOwnProperty('token') && floorInfo['floorData'].hasOwnProperty('userActivityInfo')) {
                                        ...
                                        const {
                                            activityExistFlag, redPacketOpenFlag, redPacketRewardTakeFlag, beanAmountTakeMinLimit, currActivityBeanAmount
...
function pg_interact_interface_invoke(floorToken) {
    const body = {
        floorToken, "dataSourceCode": "takeReward",
        "argMap": {}
    }
    const options = {
...
function openRedPacket(floorToken) {
        const body = {
            floorToken, "dataSourceCode": "openRedPacket",
            "argMap": {}
        }
        const options = {
            url: `${JD_API_HOST}?functionId=pg_interact_interface_invoke&body=${escape(JSON.stringify(body))}&appid=swat_miniprogram&fromType=wxapp&timestamp=${new Date().getTime() + new Date().getTimezoneOffset()*60*1000 + 8*60*60*1000}`,
    ...
    //================赚京豆开团===========
async
function distributeBeanActivity() {
    try {
        $.tuan = ''
        $.hasOpen = false;
        $.assistStatus = 0;
        ...
        if ($.tuan && $.tuan.hasOwnProperty('assistedPinEncrypted') && $.assistStatus !== 3) {
            ...
            $.http.post({
                ...
                body: JSON.stringify(Object.assign($.tuan, {
                    "time": Date.now()
                })),
                timeout: 30000
            }).then((resp) => {
...
function getRandomCode() {
        await $.http.get({
            url: `https://code.c-hiang.cn/api/v1/jd/zuan/read/${randomCount}`,
            timeout: 10000
        }).then(async(resp) => {
            if (resp.statusCode === 200) {
                try {
